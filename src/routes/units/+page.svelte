<script lang="ts">
	...
	function openModal(mode: 'add' | 'edit', unit: Unit | null = null) {
		...
		if (mode === 'edit' && unit) {
			...
			selectedUnit = { name: '', symbol: '', is_base_unit: true, base_unit_id: null };
		}
	}
...
<div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
	<div>
		<h1 class="text-2xl font-bold text-gray-800">{$t('Unit Management')}</h1>
		<p class="mt-1 text-sm text-gray-500">{$t('Manage product units')}</p>
	</div>
	<button
	...
	<table class="min-w-full divide-y divide-gray-200 text-sm">
		<thead class="bg-gray-50">
			<tr>
				...
				<th class="px-4 py-3 text-left font-semibold text-gray-600">{$t('Symbol')}</th>
				<th class="px-4 py-3 text-left font-semibold text-gray-600">{$t('Type')}</th>
				<th class="px-4 py-3 text-left font-semibold text-gray-600">{$t('Base Unit')}</th>
		...
		<tbody class="divide-y divide-gray-200 bg-white">
			{#if data.units.length === 0}
				<tr>
					<td colspan="5" class="py-12 text-center text-gray-500">{$t('No unit data found')}</td>
				</tr>
			{:else}
				{#each data.units as unit (unit.id)}
					<tr class="hover:bg-gray-50">
						<td class="px-4 py-3 font-medium text-gray-900">{unit.name}</td>
						<td class="px-4 py-3 font-mono text-xs text-gray-700 uppercase">{unit.symbol}</td>
						<td class="px-4 py-3">
						...
						<td class="px-4 py-3 text-gray-600"
							>{unit.base_unit_name ?? (unit.is_base_unit ? '-' : 'N/A')}</td
						>
						<td class="px-4 py-3">
		...
		<div class="relative w-full max-w-lg transform rounded-xl bg-white shadow-2xl">
			<div class="border-b px-6 py-4">
				<h2 class="text-lg font-bold text-gray-900">
					{modalMode === 'add' ? $t('Add New Unit') : $t('Edit unit')}
				</h2>
			</div>
			...
			>
				<div class="space-y-4 p-6">
					...
					<div class="grid grid-cols-2 gap-4">
						...
						<div>
							<label for="symbol" class="mb-1 block text-sm font-medium text-gray-700"
								>{$t('Symbol *')}</label
							>
							<input
								type="text"
								...
								id="symbol"
								required
								bind:value={selectedUnit.symbol}
					...
					<div>
						<span class="mb-1 block text-sm font-medium text-gray-700">{$t('Unit Type *')}</span>
						<div class="mt-2 flex items-center space-x-4">
							<label class="flex items-center gap-2">
								...
								/>
								<span class="text-sm">{$t('Base Unit (e.g., Piece, Kg)')}</span>
							</label>
							<label class="flex items-center gap-2">
								<input
									type="radio"
									...
									class="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500"
								/>
								<span class="text-sm">{$t('Sub Unit (e.g., Box, Dozen)')}</span>
							</label>
						</div>
						<input
							type="hidden"
							name="is_base_unit"
							value={selectedUnit.is_base_unit === true ? 'true' : 'false'}
						/>
					</div>

					{#if selectedUnit.is_base_unit === false}
						<div transition:slide>
							<label for="base_unit_id" class="mb-1 block text-sm font-medium text-gray-700"
								>{$t('Base Unit *')}</label
							>
							<select
								name="base_unit_id"
								id="base_unit_id"
								bind:value={selectedUnit.base_unit_id}
								required
								class="w-full rounded-md border-gray-300"
							>
								<option value={null}>{$t('-- Select Base Unit --')}</option>
								{#each data.baseUnits as baseUnit (baseUnit.id)}
									{#if baseUnit.id !== selectedUnit?.id}
										<option value={baseUnit.id}>{baseUnit.name} ({baseUnit.symbol})</option>
									{/if}
								{/each}
							</select>
							<p class="mt-1 text-xs text-gray-500">
								{$t('Select the main unit this sub-unit relates to.')}
							</p>
						</div>
					{/if}
	...
	>
		<div class="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
			...
			<p class="mt-2 text-sm text-gray-600">
				{$t('Are you sure you want to delete unit')} "<strong>{unitToDelete.name}</strong>"?
