...
export const actions: Actions = {
	saveMenu: async ({ request, locals }) => {
		...
		const parent_id =
			parent_id_raw && parent_id_raw !== 'null' ? parseInt(parent_id_raw, 10) : null;
		...
		try {
			const params = [title, icon, finalRoute, parent_id, permission_name || null, order];
		...
		try {
			...
			if (err.code === 'ER_ROW_IS_REFERENCED_2') {
				return fail(409, {
					success: false,
					message:
						'Cannot delete menu. It is still referenced by other database records (e.g., deeper sub-menus).',
					action: 'deleteMenu'
				});
			}
