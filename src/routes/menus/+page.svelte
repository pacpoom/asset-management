<script lang="ts">
	...
	// 1. สร้าง Interface Menu ให้ชัดเจน เพื่อแก้ปัญหา Type ตีกันกับ RowDataPacket
	interface Menu {
		id: number;
		title: string;
		icon: string | null;
		...
		order: number;
		children?: Menu[]; // เครื่องหมาย ? แปลว่ามีหรือไม่มีก็ได้
		level?: number;
	}

	// รับข้อมูลจาก Server และแปลง Type ให้เป็น Menu[] ของเรา
	const { data, form } = $props<{ data: PageData; form: ActionData }>();

	// แปลงข้อมูลดิบให้ TypeScript มองเป็น Menu[] อย่างถูกต้อง
	...
	// 2. ระบุ Type (m: Menu) ใน filter เพื่อแก้ Error 'implicit any'
	const selectableParents = $derived(
		menusData
			...
			// 3. ระบุ Type (a: Menu, b: Menu) ใน sort
			.sort((a: Menu, b: Menu) => a.order - b.order)
	);

	// Helper function: ระบุ Type ให้ parameter ทุกตัว
	function isAncestor(potentialParent: Menu, childId: number, allMenus: Menu[]): boolean {
		...
		while (currentParentId !== null) {
			if (currentParentId === childId) {
				return true; // เจอว่าวนลูป (Circular dependency)
			}
	...
	// ฟังก์ชันจัดเรียงเมนูแบบลำดับชั้น (Tree)
	function renderMenuRows(menus: Menu[], level: number = 0) {
		const rows: Menu[] = [];
		const menuMap = new Map<number, Menu>();

		// สร้าง Map และกำหนด children เป็น Array ว่างที่ระบุ Type ชัดเจน
		...
		menuMap.forEach((menu) => {
			if (menu.parent_id && menuMap.has(menu.parent_id)) {
				...
				// ตรวจสอบว่า parent และ parent.children มีอยู่จริง
		...
		// ฟังก์ชันแปลง Tree กลับเป็น Flat list เพื่อแสดงผลในตาราง
		function flatten(menuList: Menu[], currentLevel: number) {
			menuList.forEach((menu: Menu) => {
				// สร้าง Object ใหม่ที่มี level โดยไม่แก้ object เดิม
...
<div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
	<div>
		...
		<p class="mt-1 text-sm text-gray-500">
			Organize the application's navigation sidebar. (รองรับ Sub Menu)
		</p>
	</div>
	<button
		onclick={() => openModal('add')}
		class="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white"
	>
		<svg
...
<div class="overflow-x-auto rounded-lg border border-gray-200 bg-white">
	<table class="min-w-full divide-y divide-gray-200 text-sm">
		<thead class="bg-gray-50">
			<tr>
				<th class="w-1/3 px-4 py-3 text-left font-semibold text-gray-600">Title</th>
		...
		<div class="relative w-full max-w-lg transform rounded-xl bg-white shadow-2xl">
			...
			>
				<div class="max-h-[70vh] space-y-4 overflow-y-auto p-6">
					...
					<div>
						<label for="parent_id" class="mb-1 block text-sm font-medium"
							>Parent Menu (สำหรับสร้าง Sub Menu)</label
						>
						<select
							name="parent_id"
							id="parent_id"
					...
					<div class="grid grid-cols-2 gap-4">
						<div>
							...
							<input
								...
								placeholder="เช่น assets, settings"
								class="w-full rounded-md border-gray-300"
							/>
					...
					<div>
						<label for="permission_name" class="mb-1 block text-sm font-medium"
			...
			<p class="mt-2 text-sm">
				...
				{#if childrenCount > 0}
					<span class="font-bold text-red-600">
						This will also delete all {childrenCount} sub-menus under it.</span
					>
				{/if}
			</p>
			<form
