<script lang="ts">
	...
	$: ({ invoice, items, attachments, availableStatuses } = data);
	...
	async function updateStatus(e: Event) {
		...
		isSaving = true;
		await tick(); // รอให้ Svelte อัปเดตค่าลง input hidden
		if (updateStatusForm) {
	...
	<div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
		<div>
			<div class="flex items-center gap-3">
				...
				<span
					class="rounded-full px-2.5 py-0.5 text-xs font-medium {getStatusClass(invoice.status)}"
				>
					{invoice.status}
				</span>
			</div>
			<p class="mt-1 text-sm text-gray-500">
				สร้างโดย {invoice.created_by_name} เมื่อ {formatDate(invoice.created_at)}
			</p>
		</div>
	...
	<div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
		<div class="grid grid-cols-1 gap-6 border-b border-gray-200 p-6 md:grid-cols-2">
			<div>
				...
				<p class="text-lg font-medium text-gray-900">{invoice.customer_name}</p>
				<p class="text-sm whitespace-pre-line text-gray-600">{invoice.customer_address || '-'}</p>
				<p class="mt-1 text-sm text-gray-600">Tax ID: {invoice.customer_tax_id || '-'}</p>
			</div>
			<div class="md:text-right">
				...
				<div>
					<span class="text-xs font-semibold tracking-wider text-gray-500 uppercase"
						>เอกสารอ้างอิง</span
					>
					<p class="text-gray-900">{invoice.reference_doc || '-'}</p>
				</div>
			</div>
		</div>
			...
			<div class="flex flex-col justify-between gap-6 md:flex-row">
				...
				<div class="w-full space-y-2 md:w-1/3">
					...
					{#if parseFloat(invoice.withholding_tax_amount) > 0}
						<div class="flex justify-between text-sm text-red-600">
							<span>หัก ณ ที่จ่าย ({invoice.withholding_tax_rate}%)</span>
							<span>- {formatCurrency(invoice.withholding_tax_amount)}</span>
						</div>
					{/if}
					<div class="mt-1 flex items-center justify-between border-t border-gray-300 pt-3">
						...
						<span class="text-xl font-bold text-blue-600"
							>{formatCurrency(invoice.total_amount)}</span
						>
					</div>
				</div>
			</div>
