<script lang="ts">
	...
	function openModal(mode: 'add' | 'edit', user: User | null = null) {
		...
		if (mode === 'edit' && user) {
			...
			selectedUser = { role: 'user', department_id: undefined, position_id: undefined, emp_id: '' };
		}
	}
			...
			{:else}
				{#each data.users as user (user.id)}
					<tr class="hover:bg-gray-50">
						...
						<td class="whitespace-nowrap px-4 py-3 font-mono text-xs text-gray-700">{user.emp_id ?? 'N/A'}</td>
						<td class="whitespace-nowrap px-4 py-3 text-gray-600">{user.email}</td>
						<td class="whitespace-nowrap px-4 py-3 font-mono text-xs text-gray-700">{user.username}</td>
						<td class="whitespace-nowrap px-4 py-3 text-gray-600">{user.department_name ?? 'N/A'}</td>
						<td class="whitespace-nowrap px-4 py-3 text-gray-600">{user.position_name ?? 'N/A'}</td>
						<td class="whitespace-nowrap px-4 py-3">
							<span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {user.role === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'}">
								{user.role}
							</span>
						</td>
						<td class="whitespace-nowrap px-4 py-3">
		...
		<div class="relative w-full max-w-lg transform rounded-xl bg-white shadow-2xl transition-all">
			...
			>
				<div class="space-y-6 p-6">
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
						...
						<div>
							...
							<input type="text" name="full_name" id="full_name" required bind:value={selectedUser.full_name} class="w-full rounded-md border-gray-300 shadow-sm"/>
						</div>
						<!-- Employee ID -->
						<div>
							<label for="emp_id" class="mb-1 block text-sm font-medium text-gray-700">Employee ID</label>
							<input type="text" name="emp_id" id="emp_id" bind:value={selectedUser.emp_id} class="w-full rounded-md border-gray-300 shadow-sm"/>
						</div>
					</div>
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
						...
						<div>
							...
							<input type="text" name="username" id="username" required bind:value={selectedUser.username} class="w-full rounded-md border-gray-300 shadow-sm"/>
						</div>

						<!-- Email -->
						<div>
							<label for="email" class="mb-1 block text-sm font-medium text-gray-700">Email</label>
							<input type="email" name="email" id="email" required bind:value={selectedUser.email} class="w-full rounded-md border-gray-300 shadow-sm"/>
						</div>
					</div>
					...
					<div>
						...
						<input type="password" name="password" id="password" required={modalMode === 'add'} bind:value={selectedUser.password} class="w-full rounded-md border-gray-300 shadow-sm"/>
						{#if modalMode === 'edit'}
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
						...
						<div>
							...
							<select name="department_id" id="department_id" bind:value={selectedUser.department_id} class="w-full rounded-md border-gray-300 shadow-sm">
						...
						<!-- Position -->
						<div>
							<label for="position_id" class="mb-1 block text-sm font-medium text-gray-700">Position</label>
							<select name="position_id" id="position_id" bind:value={selectedUser.position_id} class="w-full rounded-md border-gray-300 shadow-sm">
					...
					<div>
						...
						<select name="role" id="role" bind:value={selectedUser.role} class="w-full rounded-md border-gray-300 shadow-sm">
				...
				<div class="flex justify-end gap-3 border-t border-gray-200 bg-gray-50 p-4">
					<button type="button" on:click={closeModal} class="rounded-md border bg-white px-4 py-2 text-sm font-medium shadow-sm hover:bg-gray-50"> Cancel </button>
					<button type="submit" disabled={isLoading} class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm disabled:bg-blue-400 hover:bg-blue-700">
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="alertdialog">
		<div class="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
			...
			<p class="mt-2 text-sm text-gray-600">
				Are you sure you want to delete user "<strong>{userToDelete.full_name}</strong>"? This action cannot be undone.
			</p>
			<form
				method="POST"
				action="?/deleteUser"
			...
			>
				...
				<button type="button" on:click={() => (userToDelete = null)} class="rounded-md border bg-white px-4 py-2 text-sm font-medium shadow-sm hover:bg-gray-50">Cancel</button>
