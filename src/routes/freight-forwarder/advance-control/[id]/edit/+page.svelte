<script lang="ts">
	...
	let attachments = data.existingAttachments || [];
	...
	// สร้าง Initial Options สำหรับดึงค่าเริ่มต้นแบบ Synchronous
	const initialBankOptions = (data.banks || []).map((b: Bank) => ({
		value: b.id,
		label: `[${b.bank_code || '-'}] ${b.bank_name}`,
		account_info: `A/C: ${b.account_number} (${b.account_name})`
	}));

	// แปลงข้อมูล Master Bank ให้เป็น Reactive เพื่อรองรับการเพิ่ม/แก้ไขจาก Modal
	...
	// ใช้ initialBankOptions แทน bankOptions ในจังหวะประกาศตัวแปร
	let selectedBank: SelectOption | null = initialBankOptions.find((b: SelectOption) => b.value == advance.bank_id) || null;
	
	let documentDate = advance.document_date ? new Date(advance.document_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
	let amount: number | string = advance.amount || '';
	let applicationTitle = advance.application_title || '';
	let reason = advance.reason || '';
	let remark = advance.remark || '';
	let status = advance.status || 'Pending';
	let previewDocNumber = advance.document_number;
...
<svelte:head>
	<title>{$t('Edit Advance')} - {previewDocNumber}</title>
</svelte:head>
		...
		<div class="flex items-center gap-4">
			...
			<div>
				<div class="flex items-center gap-2">
					<h1 class="text-xl font-bold text-gray-800">{$t('Edit Advance Application')}</h1>
				...
				<p class="text-xs text-gray-500">{$t('Update advance details')}</p>
			</div>
		</div>
		<div class="rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider
			{status === 'Pending' ? 'bg-yellow-100 text-yellow-700' : status === 'Approved' ? 'bg-blue-100 text-blue-700' : status === 'Completed' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
			STATUS: {status}
		</div>
	</div>
		...
		>
			<div class="divide-y divide-gray-100">
					...
					<div class="space-y-6 rounded-lg border border-gray-100 bg-gray-50/50 p-5">
						...
						<div class="mt-4">
							...
							{#if attachments && attachments.length > 0}
								<ul class="mb-3 space-y-2 rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
									{#each attachments as file (file.id)}
										<li class="flex items-center justify-between rounded border bg-gray-50 p-2 text-sm">
											<a
												href={file.url}
												target="_blank"
												rel="noopener noreferrer"
												class="max-w-[200px] truncate text-blue-600 hover:underline sm:max-w-[250px]"
											>
				...
				<div class="flex items-center justify-end gap-3 border-t border-gray-200 bg-white px-8 py-5">
					...
					<button type="submit" disabled={isSaving} class="flex items-center gap-2 rounded-lg bg-green-600 px-8 py-2.5 text-sm font-bold text-white shadow transition-all hover:bg-green-700 disabled:opacity-70">
						{#if isSaving}
