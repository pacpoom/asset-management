<script lang="ts">
	...
	function confirmDelete(category: any) {
		itemToDeleteId = category.id;
		itemToDeleteName = category.category_name;
...
<div class="mx-auto max-w-6xl p-6">
	<div class="mb-6 flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold text-gray-800">{$t('Expense Categories')}</h1>
			<p class="text-sm text-gray-500">{$t('Manage Expense Categories (Master Data)')}</p>
		</div>
		<button
				...
				{#each categories as cat}
					<tr class="hover:bg-gray-50">
						...
						<td class="px-6 py-4 text-center">
							<span
								class="rounded-full px-2 py-1 text-xs font-semibold {cat.is_active
									? 'bg-green-100 text-green-800'
				...
				{:else}
					<tr
						><td colspan="5" class="px-6 py-8 text-center text-gray-500"
							>{$t('No expense categories found')}</td
						></tr
					>
				{/each}
			</tbody>
		...
		<div class="w-full max-w-md overflow-hidden rounded-xl bg-white shadow-2xl">
			<div class="border-b bg-gray-50 px-6 py-4">
				<h3 class="text-lg font-bold text-gray-800">
					{modalMode === 'create' ? $t('Add New Category') : $t('Edit Category')}
				</h3>
			</div>
			...
			>
				<div class="space-y-4 p-6">
					...
					<div>
						<label for="category_code" class="mb-1 block text-sm font-semibold text-gray-700"
							>{$t('Category Code')} <span class="text-red-500">*</span></label
						>
						<input
							id="category_code"
							type="text"
							...
							placeholder="e.g. TRP, CUS"
						/>
					</div>

					<div>
						<label for="category_name" class="mb-1 block text-sm font-semibold text-gray-700"
						...
						<input
							...
							required
							class="w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500"
							placeholder={$t('e.g. ค่าขนส่ง (Transport)')}
						/>
					</div>
