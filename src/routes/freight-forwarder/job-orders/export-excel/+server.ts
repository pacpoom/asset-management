	...
	try {
		...
		const alertSql = `
			SELECT j.id, j.job_number, j.eta, j.job_status,
			       ...
			       IF(vm.demurrage_days IS NOT NULL,
			          DATEDIFF(CURDATE(), DATE_ADD(j.eta, INTERVAL vm.demurrage_days DAY)), NULL) as days_overdue_demurrage,
			       IF(vm.storage_days IS NOT NULL,
			          DATEDIFF(CURDATE(), DATE_ADD(j.eta, INTERVAL vm.storage_days  DAY)), NULL) as days_overdue_storage,
			       IF(vm.detention_days IS NOT NULL,
			          DATEDIFF(CURDATE(), DATE_ADD(j.eta, INTERVAL vm.detention_days DAY)), NULL) as days_overdue_detention
			FROM job_orders j
