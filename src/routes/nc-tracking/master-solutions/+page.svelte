<script lang="ts">
	...
	let deletingSolution: any = null;
	let isDeleting = false;

	function openDeleteModal(solution: any) {
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h1 class="text-2xl font-bold text-gray-800">{$t('รายการวิธีแก้ไข')}</h1>
		<button
...
{/if}
{#if isDeleteModalOpen && deletingSolution}
	<div
		...
		<div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
			...
			<div class="p-6 text-center text-gray-700">
				...
				<div class="rounded-lg border border-gray-100 bg-gray-50 p-3 text-sm text-gray-600">
					<p>
						{$t('วิธีแก้ไข')}: <span class="font-bold text-gray-800">{deletingSolution.name}</span>
					</p>
				</div>
			</div>
			<div class="flex justify-end gap-3 border-t bg-gray-50 px-6 py-4">
				...
				>
					<input type="hidden" name="id" value={deletingSolution.id} />
					<button
						type="submit"
						disabled={isDeleting}
