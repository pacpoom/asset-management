...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h1 class="text-2xl font-bold text-gray-800">{$t('รายการชิ้นส่วนรถ (Car Parts)')}</h1>
		<button
		...
		<table class="w-full text-left text-sm text-gray-600">
			<thead class="border-b border-gray-200 bg-gray-50 text-gray-700">
				<tr>
					...
					<th class="w-2/5 px-6 py-4 font-semibold">{$t('ชื่อชิ้นส่วน')}</th>
					<th class="px-6 py-4 font-semibold">{$t('ตำแหน่ง (Position)')}</th>
			...
			>
				...
				<div>
					<label for="part-name" class="mb-1 block text-sm font-medium text-gray-700"
						>{$t('ชื่อชิ้นส่วน')} <span class="text-red-500">*</span></label
					>
					<input
						id="part-name"
						type="text"
				...
				<div>
					...
					<input
						...
						class="w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"
						placeholder={$t('เช่น RH, LH, C, FR, RR')}
					/>
				</div>
