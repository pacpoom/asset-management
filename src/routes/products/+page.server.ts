...
interface ChartOfAccount { id: number; account_code: string; account_name: string; }
interface User { id: number; full_name: string; } // For potential assignment later, though not in schema now
...
export const load: PageServerLoad = async ({ url, locals }) => {
    ...
    try {
        ...
        const [productRows] = await pool.execute<Product[]>(productsSql, [...params, pageSize, offset]);


        // Fetch related data for dropdowns
        const [categoryRows] = await pool.execute<ProductCategory[]>('SELECT id, name FROM product_categories ORDER BY name');
        ...
        const [accountRows] = await pool.execute<ChartOfAccount[]>('SELECT id, account_code, account_name FROM chart_of_accounts WHERE is_active = 1 ORDER BY account_code');
...
export const actions: Actions = {
    saveProduct: async ({ request, locals }) => {
        ...
        if (!sku || !name || !product_type || !unit_id) {
            return fail(400, { action: 'saveProduct', success: false, message: 'SKU, Name, Product Type, and Base Unit are required.' });
        }
        ...
        try {
            ...
            if (id) {
                ...
                const sql = `UPDATE products SET
                    sku = ?, name = ?, description = ?, product_type = ?, category_id = ?, unit_id = ?,
                ...
                await connection.execute(sql, [
                    data.sku, data.name, data.description, data.product_type, data.category_id, data.unit_id,
                    data.purchase_unit_id, data.sales_unit_id, data.preferred_vendor_id, data.purchase_cost, data.selling_price,
                    data.quantity_on_hand, data.reorder_level, data.is_active, imageUrl, data.asset_account_id,
                    data.income_account_id, data.expense_account_id,
                    parseInt(id)
                ]);
            } else {
                // INSERT
            ...
            return { action: 'saveProduct', success: true, message: `Product '${data.name}' saved successfully!` };
            ...
            if (err.code === 'ER_DUP_ENTRY') {
                return fail(409, { action: 'saveProduct', success: false, message: 'A product with this SKU already exists.' });
            }
            // Add checks for FK violations if needed (though UI should prevent)
            if (err.code === 'ER_NO_REFERENCED_ROW_2') {
                 return fail(400, { action: 'saveProduct', success: false, message: 'Invalid Category, Unit, Vendor, or Account selected.' });
            }
            return fail(500, { action: 'saveProduct', success: false, message: `Failed to save product data. Error: ${err.message}` });
        } finally {
            connection.release();
        }
    },
    deleteProduct: async ({ request, locals }) => {
        ...
        try {
            ...
            if ((deleteResult as any).affectedRows === 0) {
                 await connection.rollback();
                 connection.release();
                 return fail(404, { action: 'deleteProduct', success: false, message: 'Product not found.' });
            }
            ...
            if (error.message.startsWith('Cannot delete product')) { // Catch custom error from checks
                 return fail(409, { action: 'deleteProduct', success: false, message: error.message });
            }
            if (error.code === 'ER_ROW_IS_REFERENCED_2') {
                 return fail(409, { action: 'deleteProduct', success: false, message: 'Cannot delete product. It is referenced in other records.' });
            }
			return fail(500, { action: 'deleteProduct', success: false, message: `Failed to delete product. Error: ${error.message}` });
        }
    }
};