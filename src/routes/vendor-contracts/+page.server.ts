...
async function saveFile(
	...
	try {
		...
		console.error(
			`saveFile (Vendor Contract): Error saving ${uploadPath}. ${uploadError.message}`,
			uploadError.stack
		);
		if (uploadPath) {
			try {
...
async function deleteFile(systemName: string | null | undefined) {
	...
	try {
		...
		if (error.code !== 'ENOENT') {
			console.error(
				`deleteFile (Vendor Contract): Failed ${systemName}. ${error.message}`,
				error.stack
			);
...
async function getContractDocuments(
	contractId: number,
	connection?: any
): Promise<VendorContractDocument[]> {
...
export const actions: Actions = {
	...
	saveContract: async ({ request, locals }) => {
		...
		if (!data.title || !data.vendor_id) {
			return fail(400, {
				action: 'saveContract',
				success: false,
				message: 'Title and Vendor are required.'
			});
		}
		...
		if (!id && validFiles.length === 0) {
			return fail(400, {
				action: 'saveContract',
				success: false,
				message: 'At least one document file is required for new contracts.'
			});
		}
		...
		try {
			...
			console.error(
				`Database error saving vendor contract (ID: ${contractId}): ${err.message}`,
				err.stack
			);
			...
			if (
				...
				err.message.includes("'vendor_contracts.contract_number'")
			) {
				return fail(409, {
					action: 'saveContract',
					success: false,
					message: 'This Contract Number is already in use. Please enter a unique number.'
				});
			}
			// --- MODIFICATION END ---
			...
			if (err.code === 'ER_NO_REFERENCED_ROW_2') {
				return fail(400, {
					action: 'saveContract',
					success: false,
					message: 'Invalid Vendor, User, or Contract Type selected.'
				});
			}
			return fail(500, {
				action: 'saveContract',
				success: false,
				message: err.message || 'Failed to save contract data.'
			});
		} finally {
			connection.release();
		}
	...
	deleteContract: async ({ request, locals }) => {
		...
		try {
			...
			const [deleteResult] = await connection.execute('DELETE FROM vendor_contracts WHERE id = ?', [
				contractId
			]);
			...
			await connection.rollback();
			console.error(
				`Error deleting vendor contract ID ${contractId}: ${error.message}`,
				error.stack
			);
			...
			return fail(500, {
				action: 'deleteContract',
				success: false,
				message: `Failed to delete contract. Error: ${error.message}`
			});
		} finally {
			connection.release();
		}
	...
	uploadDocument: async ({ request, locals }) => {
		...
		if (!vendor_contract_id || !documentFile || documentFile.size === 0 || !userId) {
			return fail(400, {
				...
				success: false,
				message: 'Contract ID, document file, and user session are required.'
			});
		}
		...
		try {
			...
			const newDocument = newDocRows[0]
				? {
						...newDocRows[0],
						file_path: `/uploads/vendor_contracts/${newDocRows[0].file_system_name}`
					}
				: null;
			...
			await connection.rollback();
			console.error(
				`Error uploading document for contract ${contractId}: ${err.message}`,
				err.stack
			);
