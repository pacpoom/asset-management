<script lang="ts">
	...
	function openModal(mode: 'add' | 'edit', account: ChartOfAccount | null = null) {
		...
		if (mode === 'edit' && account) {
            ...
            // Set defaults for a new account, preselect first type if available
			selectedAccount = { account_code: '', account_name: '', account_type: data.accountTypes?.[0] || '', description: '', is_active: true };
		}
	}
			...
			{:else}
				{#each data.accounts as account (account.id)}
					<tr class="hover:bg-gray-50">
						...
						<td class="px-4 py-3 text-gray-600 truncate max-w-xs" title={account.description ?? ''}>{account.description ?? '-'}</td>
                        <td class="px-4 py-3 text-center">
                            {#if account.is_active}
                                <span class="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800">✓</span>
                            {:else}
						...
						<td class="px-4 py-3 whitespace-nowrap">
							<div class="flex items-center gap-2">
								<button onclick={() => openModal('edit', account)} class="rounded p-1.5 text-gray-500 hover:bg-gray-100 hover:text-blue-600" aria-label="Edit account">
								...
								<button onclick={() => (accountToDelete = account)} class="rounded p-1.5 text-gray-500 hover:bg-gray-100 hover:text-red-600" aria-label="Delete account">
	...
	<div transition:slide class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4 pt-16">
		...
		<div class="relative w-full max-w-lg transform rounded-xl bg-white shadow-2xl">
			<div class="border-b px-6 py-4">
				<h2 class="text-lg font-bold text-gray-900">{modalMode === 'add' ? 'เพิ่มบัญชีใหม่' : 'แก้ไขบัญชี'}</h2>
			</div>

			<form method="POST" action="?/saveAccount" use:enhance={() => { isSaving = true; return async ({ update }) => { await update(); isSaving = false; }; }}>
				<div class="space-y-4 p-6 max-h-[70vh] overflow-y-auto">
					{#if modalMode === 'edit'}<input type="hidden" name="id" value={selectedAccount.id} />{/if}

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="account_code" class="mb-1 block text-sm font-medium text-gray-700">Account Code *</label>
                            <input type="text" name="account_code" id="account_code" required bind:value={selectedAccount.account_code} class="w-full rounded-md border-gray-300"/>
                        </div>
                         <div>
                            <label for="account_name" class="mb-1 block text-sm font-medium text-gray-700">Account Name *</label>
                            <input type="text" name="account_name" id="account_name" required bind:value={selectedAccount.account_name} class="w-full rounded-md border-gray-300"/>
                        </div>
                    </div>

                    <div>
                        <label for="account_type" class="mb-1 block text-sm font-medium text-gray-700">Account Type *</label>
                        <select name="account_type" id="account_type" bind:value={selectedAccount.account_type} required class="w-full rounded-md border-gray-300">
                     ...
                     <div>
                        ...
                        <textarea name="description" id="description" rows="3" bind:value={selectedAccount.description} class="w-full rounded-md border-gray-300"></textarea>
                    </div>
				...
				<div class="flex justify-end gap-3 border-t bg-gray-50 p-4">
					...
					<button type="submit" disabled={isSaving} class="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm disabled:bg-blue-400 hover:bg-blue-700">
						{#if isSaving} Saving... {:else} Save Account {/if}
					</button>
				</div>
			</form>
		</div>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="alertdialog">
		<div class="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
			...
			<p class="mt-2 text-sm text-gray-600">
                ...
                <strong class="font-mono text-xs">{accountToDelete.account_code} - {accountToDelete.account_name}</strong>?
			...
			<form method="POST" action="?/deleteAccount" use:enhance class="mt-6 flex justify-end gap-3">
				...
				<button type="button" onclick={() => (accountToDelete = null)} class="rounded-md border bg-white px-4 py-2 text-sm font-medium shadow-sm hover:bg-gray-50">Cancel</button>
