<script lang="ts">
	...
	export let data; // รับข้อมูลจาก load function
	...
	// Format Date แบบเดียวกับ Invoices
	...
	function openDeleteModal(po: any) {
		deleteId = po.id;
		deletePoNumber = po.po_number;
...
<div class="mb-6 flex items-center justify-between">
	<div>
		<h1 class="text-2xl font-bold text-gray-800">ใบสั่งซื้อ (Purchase Orders)</h1>
		<p class="mt-1 text-sm text-gray-500">จัดการรายการสั่งซื้อสินค้าจากผู้ขาย</p>
	</div>
	<a
...
>
	<div class="relative md:col-span-1">
		...
		<input
			...
			placeholder="ค้นหาเลขที่เอกสาร หรือ ชื่อผู้ขาย..."
		...
		<table class="min-w-full divide-y divide-gray-200 text-sm">
			<thead class="bg-gray-50">
				<tr>
					...
					<th class="px-4 py-3 text-left font-semibold text-gray-600">ผู้ขาย (Vendor)</th>
			...
			<tbody class="divide-y divide-gray-200 bg-white">
				{#each data.purchaseOrders as po}
					<tr class="transition-colors hover:bg-gray-50">
						<td class="px-4 py-3 font-medium text-blue-600">
							<a href="/purchase-orders/{po.id}">{po.po_number}</a>
						</td>
						<td class="px-4 py-3 whitespace-nowrap text-gray-600">{formatDate(po.po_date)}</td>
						<td class="px-4 py-3 font-medium text-gray-900">{po.vendor_name}</td>
						<td class="px-4 py-3 text-right font-semibold text-gray-900"
							>{formatCurrency(Number(po.total_amount))}</td
						>
						...
						<td class="px-4 py-3 text-center">
							<div class="flex justify-center gap-2">
								...
								{:else}
									<span class="cursor-not-allowed text-gray-200" title="ต้องเปลี่ยนสถานะก่อนพิมพ์">
										<svg
											xmlns="http://www.w3.org/2000/svg"
											viewBox="0 0 20 20"
											fill="currentColor"
				...
				{:else}
					<tr>
						<td colspan="6" class="py-8 text-center text-gray-500">ไม่พบรายการใบสั่งซื้อ</td>
					</tr>
				{/each}
			</tbody>
		</table>
	...
	>
		...
		>
			...
			<p class="text-sm text-gray-500">
				คุณแน่ใจหรือไม่ที่จะลบใบสั่งซื้อ <strong>{deletePoNumber}</strong>? <br />
