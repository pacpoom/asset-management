<script lang="ts">
	...
	$: ({ billingNotes, currentPage, totalPages, searchQuery, filterStatus } = data);
...
<svelte:head>
	<title>ใบวางบิล (Billing Notes)</title>
</svelte:head>

<div class="mb-6 flex items-center justify-between">
	<div>
		<h1 class="text-2xl font-bold text-gray-800">ใบวางบิล (Billing Notes)</h1>
...
>
	...
	<div>
		...
		>
			...
			<option value="Sent">Sent (วางบิลแล้ว)</option>
		...
		<table class="min-w-full divide-y divide-gray-200 text-sm">
			<thead class="bg-gray-50">
				<tr>
					...
					<th class="px-4 py-3 text-left font-semibold text-gray-600">วันที่วางบิล</th>
			...
			<tbody class="divide-y divide-gray-200 bg-white">
				{#if billingNotes.length === 0}
					<tr>
						<td colspan="7" class="py-8 text-center text-gray-500">ไม่พบใบวางบิล</td>
					</tr>
				{:else}
					{#each billingNotes as note}
						<tr class="transition-colors hover:bg-gray-50">
							<td class="px-4 py-3 font-medium text-blue-600">
								<a href="/billing-notes/{note.id}">{note.billing_note_number || '(Draft)'}</a>
							</td>
							<td class="px-4 py-3 whitespace-nowrap text-gray-600"
								>{formatDate(note.billing_date)}</td
							>
							<td class="px-4 py-3 whitespace-nowrap text-gray-600">{formatDate(note.due_date)}</td>
							<td class="px-4 py-3 font-medium text-gray-900">{note.customer_name}</td>
	...
	>
		...
		>
			...
			<p class="text-sm text-gray-500">
				คุณแน่ใจหรือไม่ที่จะลบใบวางบิล <strong>{itemToDelete?.billing_note_number}</strong>?
			</p>
