...
<svelte:head>
	<title>{$t('Billing Note')} {billingNote.billing_note_number}</title>
</svelte:head>
	...
	<div class="flex items-center">
		...
		<div>
			<h1 class="text-2xl font-bold text-gray-800">
				{$t('Billing Note Details #')}{billingNote.billing_note_number}
			</h1>
			<p class="mt-1 text-sm text-gray-500">
				{$t('Customer')}: <span class="font-medium text-gray-700">{billingNote.customer_name}</span>
			</p>
		</div>
	</div>
...
<div class="mb-6 rounded-lg border bg-white p-6 shadow-sm">
	<div class="flex flex-col justify-between gap-4 border-b pb-4 md:flex-row">
		...
		<div class="text-left md:text-right">
			<h1 class="text-2xl font-bold text-gray-800 uppercase">{$t('Billing Note')}</h1>
	...
	<div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
		...
		<div class="md:col-span-1">
			<h3 class="text-sm font-semibold text-gray-500 uppercase">{$t('Due Date Info')}</h3>
			<div class="mt-2">
	...
	<div class="overflow-x-auto">
		<table class="min-w-full divide-y divide-gray-200 text-sm">
			<thead class="bg-gray-50">
				<tr>
					<th class="px-4 py-4 text-left font-medium text-gray-500">{$t('Description')}</th>
					<th class="w-[80px] px-4 py-4 text-right font-medium text-gray-500">{$t('Qty')}</th>
					<th class="w-[100px] px-4 py-4 text-right font-medium text-gray-500">{$t('Unit')}</th>
					<th class="px-4 py-4 text-right font-medium text-gray-500">{$t('Unit Price')}</th>
					<th class="px-4 py-4 text-center font-medium text-red-500">WHT</th>
					<th class="px-4 py-4 text-right font-medium text-gray-500">{$t('Total')}</th>
				</tr>
			</thead>
			<tbody class="divide-y divide-gray-200 bg-white">
				{#each items as item}
					<tr>
						<td class="px-4 py-3 font-medium text-gray-700">{item.description}</td>
						<td class="px-4 py-3 text-right text-gray-700">{item.quantity}</td>
						<td class="px-4 py-3 text-right text-gray-600">{item.unit_symbol || '-'}</td>
						<td class="px-4 py-3 text-right text-gray-700">{formatCurrency(item.unit_price)}</td>
...
<div class="mb-6 grid grid-cols-1 gap-6">
	<div class="rounded-lg border bg-white p-4 shadow-sm">
		...
		<p class="text-sm whitespace-pre-wrap text-gray-600">{billingNote.notes || $t('No notes.')}</p>
	</div>
</div>
