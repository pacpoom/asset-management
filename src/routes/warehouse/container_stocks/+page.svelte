    ...
    const filteredStocks = $derived(
        data.stocks?.filter((stock: any) => {
            ...
            if (appliedSearchTerm.trim()) {
                const searchStr = `${stock.container_no} ${stock.plan_no} ${stock.location_code}`.toLowerCase();
...
<div class="mx-auto max-w-7xl p-6">
    ...
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">{$t('Manage and view containers currently in the yard')}</p>
        </div>
        
        <!-- Search Box & Export -->
        <div class="flex flex-col sm:flex-row gap-3 lg:items-end flex-1 justify-end">
            <div class="relative w-full sm:max-w-xs xl:max-w-sm">
                ...
                <input 
                    ...
                    class="block w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-3 text-sm placeholder-gray-500 shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
                >
            </div>

            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <div class="w-full sm:w-36">
                    <label for="checkinFrom" class="block text-xs font-medium text-gray-700 mb-1">{$t('Check-in Date (From)')}</label>
                    <input
                        id="checkinFrom"
                        type="date"
                        bind:value={checkinFrom}
                        class="block w-full rounded-lg border border-gray-300 bg-white py-2.5 px-3 text-sm shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
                    />
                </div>
                <div class="w-full sm:w-36">
                    <label for="checkinTo" class="block text-xs font-medium text-gray-700 mb-1">{$t('Check-in Date (To)')}</label>
                    <input
                        id="checkinTo"
                        type="date"
            ...
            <div class="flex items-center gap-2 mt-2 sm:mt-0">
                <button
                    type="button"
                    onclick={applySearch}
                    class="bg-blue-600 text-white px-4 py-2.5 text-sm font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-sm whitespace-nowrap"
                >
                    {$t('Search')}
                </button>
                ...
                <a 
                    ...
                    target="_blank"
                    class="bg-green-600 text-white px-4 py-2.5 text-sm font-bold rounded-lg hover:bg-green-700 transition-colors shadow-sm flex items-center gap-2 whitespace-nowrap"
                >
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-left text-sm">
                <thead class="bg-gray-50 text-gray-600">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 font-semibold">{$t('Check-in Date')}</th>
                        <th scope="col" class="px-6 py-4 font-semibold">{$t('Stock Days')}</th>
                        ...
                        {#each pagedStocks as stock}
                            ...
                            <tr class="transition-colors hover:bg-gray-50">
                                ...
                                <td class="whitespace-nowrap px-6 py-4">
                                    <span class="inline-flex rounded bg-blue-50 px-2 py-1 text-xs font-mono font-semibold text-blue-700">
                                        {stock.location_code}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    <span class="inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold {badge.classes}">
                                        {$t(badge.label)}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-gray-600">
                                ...
                                <td class="max-w-xs truncate px-6 py-4 text-gray-500" title={stock.remarks}>
                                    {stock.remarks || '-'}
                                </td>
                            </tr>
                        {/each}
        ...
        {#if filteredStocks.length > pageSize}
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-gray-200 bg-white px-6 py-3">
                <div class="flex items-center gap-3">
                    <span class="text-sm text-gray-700">{$t('Show')}</span>
                    <select
                        class="rounded-md border border-gray-300 py-1 pr-8 pl-3 text-sm bg-white"
                ...
                <div class="flex items-center gap-3">
                    <span class="text-sm text-gray-600">
                        {startItem} - {endItem} {$t('from')} {filteredStocks.length}
                    </span>
                    <div class="inline-flex items-center gap-1">
                        <button
                            class="px-2 py-1 text-sm rounded-md border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50"
