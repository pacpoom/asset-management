<script lang="ts">
	...
	function selectLocationFromModal(loc: any) {
		...
		showToast(`เลือก Location: ${loc.location_code} สำเร็จ`, 'success');
		setTimeout(() => barcodeInputEl?.focus(), 100);
	}
	...
	>
		...
		>
			<div class="flex items-center justify-between border-b px-6 py-4">
				<h2 class="text-lg font-bold text-gray-900">{$t('ค้นหาและเลือก Location')}</h2>
				<button
					type="button"
					onclick={() => (showLocationModal = false)}
...
<!-- Modal สำหรับยืนยันจำนวน (Qty) รับเข้า -->
