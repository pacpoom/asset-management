...
<div class="mx-auto max-w-3xl">
	...
	<div class="mb-6">
		...
		<p class="mt-1 text-sm text-gray-500">
			{$t('สแกนตัดสต็อกออก / จ่ายของ (Location ต้นทาง -> Barcode)')}
		</p>
	</div>
	...
	<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
		...
		<div class="mb-8">
			<label for="location_input" class="mb-2 block text-sm font-semibold text-gray-700">
				1. {$t('สแกน Location')} <span class="font-bold text-red-600">{$t('ต้นทาง')}</span>
			...
			<div class="flex items-center gap-3">
				<div class="relative flex-1">
					...
					<input
						...
						placeholder={$t('ระบุรหัส Location ที่จะไปหยิบของ (เช่น A-01-01)')}
						class="w-full rounded-lg border-gray-300 py-3 pr-4 pl-10 text-lg uppercase focus:border-red-500 focus:ring-red-500 disabled:bg-gray-100 disabled:text-gray-500"
					/>
				</div>
				{#if isLocationLocked}
					<button
			...
			<div class="relative">
				...
				<input
					...
					placeholder={isLocationLocked
						? $t('สแกนบาร์โค้ดสินค้าที่ต้องการตัดสต็อกที่นี่...')
						: $t('กรุณายืนยัน Location ต้นทางก่อน')}
					class="w-full rounded-lg border-gray-300 py-4 pr-4 pl-10 font-mono text-lg focus:border-red-500 focus:ring-red-500 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400"
				/>
			</div>
		</div>
	</div>
			...
			>
				...
				<div class="mb-2">
					...
					<input
						...
						class="w-full rounded-lg border-gray-300 px-4 py-3 text-center text-xl font-bold text-red-600 focus:border-red-500 focus:ring-red-500"
					/>
				</div>
