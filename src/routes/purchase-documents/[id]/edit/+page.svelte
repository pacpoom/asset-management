<script lang="ts">
	...
	$: ({ document, existingItems, existingAttachments, vendors, products, units } = data);
	...
	>
		...
		<div class="mb-6">
			...
			<button
				type="button"
				on:click={addItem}
				class="mt-2 text-sm font-medium text-indigo-600 hover:text-indigo-800">+ เพิ่มรายการ</button>
		</div>
		...
		<div class="mb-6 flex flex-col gap-6 md:flex-row">
			...
			<div class="h-fit w-full space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-4 shadow-inner md:w-1/3">
				...
				<div class="flex items-center justify-between border-b border-gray-200 pb-2 text-sm text-red-600">
					<span class="font-medium">หัก ณ ที่จ่ายรวม (Total WHT)</span><span class="font-bold">-{whtAmount.toFixed(2)}</span>
