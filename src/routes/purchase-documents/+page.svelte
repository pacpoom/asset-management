...
<svelte:head>
	<title>เอกสารการจัดซื้อ (Purchase Documents)</title>
</svelte:head>

<div class="mb-6 flex items-center justify-between">
	<div>
		<h1 class="text-2xl font-bold text-gray-800">เอกสารการจัดซื้อและจ่ายเงิน (P2P)</h1>
		<p class="mt-1 text-sm text-gray-500">จัดการใบขอซื้อ สั่งซื้อ รับของ ตั้งหนี้ และจ่ายเงิน</p>
	</div>
	<a
...
<div class="mb-6 grid grid-cols-1 gap-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-4">
	<div class="relative md:col-span-2">
		<input
			...
			placeholder="ค้นหาเลขที่เอกสาร หรือ ชื่อผู้จำหน่าย(Vendor)..."
		...
		<table class="min-w-full divide-y divide-gray-200 text-sm">
			<thead class="bg-gray-50">
				<tr>
					...
					<th class="px-4 py-3 text-left font-semibold text-gray-600">ผู้จำหน่าย (Vendor)</th>
			...
			<tbody class="divide-y divide-gray-200 bg-white">
				{#if documents.length === 0}
					<tr>
						<td colspan="7" class="py-8 text-center text-gray-500">ไม่พบเอกสารจัดซื้อ</td>
					</tr>
				{:else}
					{#each documents as doc}
						<tr class="transition-colors hover:bg-gray-50">
							<td class="px-4 py-3">
								<span class="inline-flex items-center rounded-md border px-2 py-1 text-xs font-medium {getDocTypeColor(doc.document_type)} border-current opacity-80">
							...
							<td class="px-4 py-3 text-center">
								<div class="flex justify-center gap-2">
									<a href="/purchase-documents/{doc.id}" class="text-gray-400 transition-colors hover:text-indigo-600" title="ดูรายละเอียด">
									...
									<a href="/purchase-documents/{doc.id}/edit" class="text-gray-400 transition-colors hover:text-yellow-600" title="แก้ไข">
									...
									<button type="button" on:click={() => openDeleteModal(doc)} class="text-gray-400 transition-colors hover:text-red-600" title="ลบ">
	...
	<div class="bg-opacity-75 fixed inset-0 z-50 flex items-center justify-center bg-gray-500 p-4 transition-opacity">
		<div class="w-full transform overflow-hidden rounded-lg bg-white p-6 shadow-xl transition-all sm:max-w-lg">
			...
			<p class="text-sm text-gray-500">
				คุณแน่ใจหรือไม่ที่จะลบเอกสารจัดซื้อ <strong>{itemToDelete?.document_number}</strong>?
			</p>
