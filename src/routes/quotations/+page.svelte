					...
					{#each quotations as quote}
						<tr class="transition-colors hover:bg-gray-50">
							<td class="px-4 py-3 font-medium text-blue-600">
								<a href="/quotations/{quote.id}">{quote.quotation_number || '(Draft)'}</a>
							</td>
							<td class="px-4 py-3 whitespace-nowrap text-gray-600"
								>{formatDate(quote.quotation_date)}</td
							>
							<td class="px-4 py-3 whitespace-nowrap text-gray-600"
								>{formatDate(quote.valid_until)}</td
							>
							<td class="px-4 py-3 font-medium text-gray-900">{quote.customer_name}</td>
							<td class="px-4 py-3 text-right font-semibold text-gray-900"
								>{formatCurrency(quote.total_amount)}</td
							>
							...
							<td class="px-4 py-3 text-center">
								<div class="flex justify-center gap-2">
									...
									{#if quote.status !== 'Draft'}
										<a
											href="/quotations/generate-pdf?id={quote.id}"
											target="_blank"
