...
interface ReceiptData extends RowDataPacket {
	id: number;
...
function getReceiptHtml(
	...
	const totalAfterDiscount = receiptData.total_after_discount || 0;
	const vatRate = receiptData.vat_rate || 0;
	const vatAmt = receiptData.vat_amount || 0;
		...
		<table style="width: 100%; border-collapse: collapse; margin-bottom: 1rem; font-size: 9pt;">
			<tr style="border-bottom: 1px solid #dee2e6;">
				...
				<td style="width: 40%; vertical-align: top; text-align: right; padding-bottom: 1rem;">
					<h1 style="font-size: 1.5rem; font-weight: bold; text-transform: uppercase; margin: 0;">ใบเสร็จรับเงิน</h1>
					<p style="font-size: 10pt; color: #666;">RECEIPT</p>
					<div style="margin-top: 0.5rem; font-size: 8pt; line-height: 1.5;">
						<p style="margin:0;"><span style="font-weight: 600;">เลขที่ / No.:</span> ${receiptData.receipt_number}</p>
						<p style="margin:0;"><span style="font-weight: 600;">วันที่ / Date:</span> ${formatDateOnly(receiptData.receipt_date)}</p>
						<p style="margin:0;"><span style="font-weight: 600;">อ้างอิง / Ref:</span> ${receiptData.reference_doc || '-'}</p>
					</div>
				</td>
			</tr>
			<tr>
				<td style="padding-top: 1rem; vertical-align: top;">
					...
					<p style="font-weight: bold; margin: 0 0 4px 0;">${receiptData.customer_name}</p>
					<div style="font-size: 8pt; line-height: 1.4;">
						<p style="margin:0;">${receiptData.customer_address || '-'}</p>
					</div>
					<p style="font-size: 8pt; margin:4px 0 0 0;"><span style="font-weight: 600;">Tax ID:</span> ${receiptData.customer_tax_id || '-'}</p>
				</td>
				<td style="padding-top: 1rem; vertical-align: top; text-align: right;">
					...
					<p style="font-size: 8pt;">${receiptData.created_by_name}</p>
				</td>
			</tr>
		</table>
	`;
	...
	const signatureBlock = `
		<div style="display: flex; justify-content: space-between; margin-top: 30px; padding-top: 20px; font-size: 8pt;">
			<div style="text-align: center; width: 30%;">
				...
				<p style="margin-top: 5px;">ผู้รับเงิน (Collector)</p>
				<p>วันที่ ...../...../.....</p>
			</div>
			<div style="text-align: center; width: 30%;">
				<div style="border-bottom: 1px dotted #ccc; height: 30px;"></div>
