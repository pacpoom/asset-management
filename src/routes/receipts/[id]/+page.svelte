<script lang="ts">
	...
	$: ({ receipt, items, attachments } = data);
	...
	<div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
		<div>
			<div class="flex items-center gap-3">
				<h1 class="text-2xl font-bold text-gray-800">{receipt.receipt_number}</h1>
				<span
			...
			<p class="mt-1 text-sm text-gray-500">
				สร้างโดย {receipt.created_by_name} เมื่อ {formatDate(receipt.created_at)}
			</p>
		</div>
	...
	<div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
		<div class="grid grid-cols-1 gap-6 border-b border-gray-200 p-6 md:grid-cols-2">
			<div>
				...
				<p class="text-lg font-medium text-gray-900">{receipt.customer_name}</p>
				<p class="text-sm whitespace-pre-line text-gray-600">{receipt.customer_address || '-'}</p>
				<p class="mt-1 text-sm text-gray-600">Tax ID: {receipt.customer_tax_id || '-'}</p>
			</div>
			<div class="md:text-right">
				<div class="mb-2">
					<span class="text-xs font-semibold tracking-wider text-gray-500 uppercase"
						>วันที่เอกสาร</span
					>
					<p class="font-medium text-gray-900">{formatDate(receipt.receipt_date)}</p>
				</div>
				<div>
					<span class="text-xs font-semibold tracking-wider text-gray-500 uppercase"
						>เอกสารอ้างอิง</span
					>
					<p class="text-gray-900">{receipt.reference_doc || '-'}</p>
				</div>
			</div>
		</div>
			...
			<div class="flex flex-col justify-between gap-6 md:flex-row">
				...
				<div class="w-full space-y-2 md:w-1/3">
					<div class="flex justify-between text-sm">
						...
						<span class="font-medium text-gray-900">{formatCurrency(receipt.subtotal)}</span>
					</div>
					...
					{/if}
					<div class="flex justify-between text-sm">
						<span class="text-gray-600">VAT ({receipt.vat_rate}%)</span>
						<span class="font-medium text-gray-900">{formatCurrency(receipt.vat_amount)}</span>
					</div>
					{#if parseFloat(receipt.withholding_tax_amount) > 0}
						<div class="flex justify-between text-sm text-red-600">
							<span>หัก ณ ที่จ่าย ({receipt.withholding_tax_rate}%)</span>
					...
					<div class="mt-1 flex items-center justify-between border-t border-gray-300 pt-3">
						...
						<span class="text-xl font-bold text-blue-600"
							>{formatCurrency(receipt.total_amount)}</span
						>
					</div>
				</div>
			</div>
