...
<svelte:head>
	<title>{$t('Receipt')} #{receipt.receipt_number}</title>
</svelte:head>
...
>
	<div class="flex items-center">
		<a href="/receipts" class="mr-3 text-gray-500 hover:text-gray-800" title={$t('Back to list')}>
			<svg
		...
		<div>
			<h1 class="text-2xl font-bold text-gray-800">
				{$t('Receipt Details #')}{receipt.receipt_number}
			</h1>
			<p class="mt-1 text-sm text-gray-500">
				{$t('Customer')}: <span class="font-medium text-gray-700">{receipt.customer_name}</span> |
				{$t('Reference')}: {receipt.reference_doc || '-'}
			</p>
		</div>
	</div>
...
<div class="mb-6 rounded-lg border bg-white p-6 shadow-sm">
	<div class="flex flex-col justify-between gap-4 border-b pb-4 md:flex-row">
		...
		<div class="text-left md:text-right">
			<h1 class="text-2xl font-bold text-gray-800 uppercase">{$t('Receipt')}</h1>
...
<div class="mb-6 grid grid-cols-1 gap-6">
	<div class="rounded-lg border bg-white p-4 shadow-sm">
		...
		<p class="text-sm whitespace-pre-wrap text-gray-600">{receipt.notes || $t('No notes.')}</p>
	</div>
