<script lang="ts">
	...
	// ใช้ Reactive Statement เพื่อ Initialize ค่าครั้งแรกเมื่อ data มาถึง
	$: if (receipt) {
		receiptDate = new Date(receipt.receipt_date).toISOString().split('T')[0];
		discountAmount = parseFloat(receipt.discount_amount);
		vatRate = parseFloat(receipt.vat_rate);
		whtRate = parseFloat(receipt.withholding_tax_rate);
	}
